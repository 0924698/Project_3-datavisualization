...
public class MonumentTheft {
    ...
    public String updateDatabase(){
    	JSONObject data = mysql.query("SELECT `UNIEKNUMME` FROM monuments WHERE lat IS NULL or '';");
...
//    			System.out.println(arr.getJSONObject(i).get);
	    		...
	    		int id =arr.getJSONObject(i).getInt("unieknumme");
	    		
	    		System.out.println(id);
	    		
	    		JSONArray fullData = mysql.query("SELECT * FROM monuments WHERE `unieknumme`='" + id + "';").getJSONArray("data");
	    		JSONObject obj = fullData.getJSONObject(0);
	    		
	    		
	    		String url =  base + convertSpacesToPlus( "address=" + obj.getString("naam_adres") + ", ROTTERDAM" )+ "&key=" + apikey;
